...
#include <sys/stat.h>
#ifdef _WIN32
/* MinGW/MSVC have no two-arg POSIX mkdir. */
#include <direct.h>
...
/* Build a syntactically valid TMD: 1 object, `nv` verts, `np` untextured
 * gouraud tri prims in SH1's shipped shape (mode 0x30, ilen 4 -> 20 bytes
 * each; the validator rejects any packet shape the port's dispatch would
 * mis-stride), padded out to at least `padTo` bytes. */
static size_t build_tmd(u8* out, u32 id, u32 nobj, u32 nv, u32 np, size_t padTo)
{
    const u32 primBytes = np * 20u;
    ...
    for (i = 0; i < np; i++)
    {
        u8* p = out + 12 + primtop + i * 20;
        p[0] = 6;    /* olen, as shipped for mode 0x30 */
        p[1] = 4;    /* ilen -> 4 + 16 = 20 bytes */
        p[2] = 0x00; /* flag: no-light, single-sided (stock) */
        p[3] = 0x30; /* mode: untextured gouraud tri */
    }
...
int main(void)
{
    ...
    mkdir("gamedata", 0755); /* may already exist */
    ...
    /* --- 5. malformed oversized files are rejected ------------------------ */
    {
        ...
        /* lit-flagged packets: valid walk, but the port's no-light dispatch
         * would mis-stride the handler batch (the SHModelViewer crash class) */
        n = build_tmd(buf, 0x41, 1, 900, 1800, 0);
        buf[12 + 28 + 2] = 0x01; /* first prim's flag: light calc ON */
        write_file(P21, buf, n + 8);
        r = Pc_BigTmd_Redirect(UNQ21, g_slab);
        assert(r == g_slab);
        printf("5.6 malformed (%-10s) -> rejected, identity OK\n", "lit packet");
    }

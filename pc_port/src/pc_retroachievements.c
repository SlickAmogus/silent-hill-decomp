...
static void Pc_RaBuildMapAuto(void)
{
    ...
    /* D_<address> globals the decomp defines but its sym files never name. An
     * unnamed range is worse than a missing one: the preceding symbol's span is
     * inferred all the way to the next NAMED entry and swallows it, so the read
     * silently lands on unrelated native memory. That is what made the whole
     * `.bss, ranking` subsegment (0x800C48A0) read 0 and kept the star
     * achievements from ever firing. Generated by
     * pc_port/tools/gen_ra_extra_syms.py; the name encodes the address. */
    Pc_RaLoadSymbolFile("gamedata/ra/sym.extra.txt", &found, &missing);
    if (s_mapCount <= 0)
        return;

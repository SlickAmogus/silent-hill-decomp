...
#include "pc_playas.h"
/* Xbox: no pc_big_lm. That module exists to lift the loose-file byte-replace
 * size gate for OVERSIZED LM replacements, which this port does not support (a
 * bigger-than-original LM would need structural validation the Xbox copy of
 * ipd_reformat.c does not have). Only its tail slack was ever needed here. */
#define PLAYAS_ILM_TAIL_SLACK 16
...
void* Pc_PlayAs_PlayerLmRedirect(int modelFileIdx, void* lmHdr)
{
    ...
    /* The player always gets this PRIVATE buffer. Disc reads are sector-granular
     * (ALIGN(fileSize, 2048) = 16 KiB for a humanoid ILM) and HELD_ITEM_LM_BUFFER
     * sits only 14592 bytes above HARRY_LM_BUFFER, so a swapped read through the
     * PSX slab overruns into the equipped weapon's PLM window. Identical layout
     * on Xbox, so the buffer is not optional.
     *
     * Xbox: no pc_big_lm. That module only exists to lift the loose-file
     * byte-replace size gate for OVERSIZED replacements, which this port does not
     * support (a bigger-than-original LM would need the reformat validation the
     * Xbox copy of ipd_reformat.c does not have). Sector-aligned size is the
     * whole requirement here. */
    (void)loose;
    need = (size_t)Fs_GetFileSectorAlignedSize(modelFileIdx) + PLAYAS_ILM_TAIL_SLACK;
    ...
    if (s_playerIlmBuf == NULL || s_playerIlmCap < need)
    {
        ...
        {
            /* PC falls back to lmHdr here. On Xbox that is the PSX slab, i.e.
             * exactly the overrun this buffer exists to prevent — and with
             * ~1.3MB free a failed calloc is a real outcome, not a theoretical
             * one. REFUSE the swap instead of corrupting the held-item window. */
            SH_DBG("[PLAYAS] calloc(%u) failed — refusing swap, reverting to Harry",
                   (unsigned)need);
            PlayAs_RowApply(0);   /* index 0 is Harry: restores the stock rows */
            s_current = 0;
            return lmHdr;
        }
    }
...
static void PlayAs_LoadSkinBinds(void)
{
    ...
    /* Xbox: this stages the WHOLE anim file (LS.ANM is 114,688 B, PRS.ANM
     * 112,640 B) just to keep ~650 bytes of bind table at 0x14. It is transient
     * — freed a few lines below — but it is the largest allocation in the
     * feature and it lands when free RAM is ~1.3MB. Refuse rather than fail
     * mid-swap: the retarget then degrades to Harry's own bone proportions,
     * which reads as slightly-off limb lengths, not a corruption. */
    if (Xbox_MemFreeKB() < (unsigned)((size / 1024) + 384))
    {
        SH_DBG("[PLAYAS] skin binds: %dKB staging needs more headroom (free=%uKB)",
               (int)(size / 1024), Xbox_MemFreeKB());
        return;
    }
...
{
    /* The skin reaches the screen through a VIRTUAL POOL SLOT, and on Xbox the
     * only decoder for those is the paletted cache (psx_vram.c). With
     * texture_paletted=0 — the documented escape hatch — xbox_respool.c reserves
     * zero slabs and the lookup resolves to nothing, so the player would render
     * as an invisible body. Refuse the swap rather than ship that. */
    if (!g_PcConfig.xboxPalettedTex)
    {
        if (g_PcConfig.playerCharacter[0] != '\0' &&
            strcmp(g_PcConfig.playerCharacter, "harry") != 0)
        {
            SH_DBG("[PLAYAS] texture_paletted=0 — '%s' unavailable, playing as Harry",
                   g_PcConfig.playerCharacter);
        }
        PlayAs_RowApply(0);
        s_current = 0;
        return;

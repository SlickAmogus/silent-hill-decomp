...
void PcConfig_Load(const char* path)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        {
            g_PcConfig.playAsL3 = (atoi(value) != 0);
        }
        else if (strcmp(key, "player_character") == 0)
        {
            strncpy(g_PcConfig.playerCharacter, value,
                    sizeof(g_PcConfig.playerCharacter) - 1);
            g_PcConfig.playerCharacter[sizeof(g_PcConfig.playerCharacter) - 1] = 0;
        }
        else if (strcmp(key, "hires_textures") == 0)
        {
            g_PcConfig.hiResTextures = (atoi(value) != 0);
        }

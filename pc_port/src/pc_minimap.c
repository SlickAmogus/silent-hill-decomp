 ...
 * replacements), which Xbox does not build. mm_pack_register already early-outs
...
int TexPack_LastComposeIsDds(void) { return 0; }
const unsigned char* TexPack_LastComposeDds(size_t* outSize) { if (outSize) *outSize = 0; return 0; }

/* Loose-file image overrides are the PC texture-pack feature; Xbox has no
 * equivalent, so the minimap simply loads the area's paper-map TIM off the disc
 * image. Stubbed rather than #ifdef'd around every use site so the loader below
 * stays one code path. */
static int            Pc_LooseImageOverridePath(s32 idx, char* out, unsigned cap) { (void)idx; (void)out; (void)cap; return 0; }
static unsigned char* Pc_LooseSlurp(const char* path, long* outSize)             { (void)path; (void)outSize; return 0; }
#else

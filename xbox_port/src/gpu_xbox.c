...
static void FbReadback(int fromLastQueued, int minSpacing)
{
    ...
    /* Direct strided read from the back buffer. A "burst-copy the whole span into
     * a cached row first" attempt (e43b0cdda) was 6x SLOWER (readMs 20->118): CPU
     * reads of the write-combined back buffer do NOT read-combine, so copying the
     * full bx0..bxN span just read ~2x the words at the same per-word stall. The
     * strided read touches only the sampled pixels — that is the CPU-readback
     * floor (~20ms for 320x224); a real speedup needs a GPU-side blit, not a
     * cleverer CPU read. */
    for (k = 0; k < pageH; k++) {

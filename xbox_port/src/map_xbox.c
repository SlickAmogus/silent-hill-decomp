 ...
 * pass is not ported there yet, so none of the prefixed symbols exist to
 * declare. Leaving the other slots NULL is the mechanism this file already has
 * for an unlinked overlay -- MapXbox_OverlayIsLinked() refuses the transition
 * and MapXbox_LogUnlinkedOverlay() reports it. Fabricating zeroed headers to
 ...
 *
 * PS3 HAS the prefixing pass (ps3_port/build_maps.sh), so it takes the full
 * registry below. Without it the attract demo could not run at all: it wants
 * map2_s00 / map1_s01 / map1_s02 and every transition was refused, which is why
 * the first frames had no world geometry in them to render. */

...
/*
 * cd_xbox360.c - PSX CD-ROM (libcd) on Xbox 360, backed by the game's BIN disc
 * image on USB or the internal drive.
 *
 * Port of xbox_port/src/cd_xbox.c. The sector logic is identical and deliberately
 * so: CdControl(CdlSetloc) sets the read sector, CdRead copies the 2048-byte user
 * data out of each 2352-byte Mode-2/Form-1 sector (payload at offset 24: 12 sync
 * + 3 addr + 1 mode + 8 subheader). The PSX async model collapses to synchronous
 * reads, so CdReadSync/CdSync report complete immediately.
 *
 * What differs from Xbox is only how the BIN is FOUND: no nxdk drive mounting,
 * no FindFirstFile. fs_xbox360.c has already picked the volume and filename.
 *
 * Self-contained CdlLOC + Cdl* constants, matching the Xbox port's reasoning:
 * this TU never includes <libcd.h>, so it cannot clash with the decomp types.
 * C links by name and CdlLOC is the standard {minute,second,sector,track} BCD
 * layout, so it matches the game's libcd prototypes at the ABI level.
 */
#include <stdio.h>
#include <string.h>
...
char g_CdBinPath[SH360_PATH_MAX] = "NOT FOUND";

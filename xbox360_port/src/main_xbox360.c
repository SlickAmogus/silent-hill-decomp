 ...
 * not return and a hang inside it must still leave the boot trace on the stick.
 * A hardware test costs a BadUpdate run, so the log has to survive the failure
...
/* sh_log_xbox360.c (port-specific, so not in the shared sh_log.h). */
...
int main(void)
{
    char cfgPath[SH360_PATH_MAX * 2];
    char pvd[8];
    int  haveBin;

    xenos_init(VIDEO_MODE_AUTO);
    console_init();
    printf("Silent Hill - Xbox 360\n");

    /* USB must enumerate before libfat can see the stick to mount uda:. */
    usb_init();
    usb_do_poll();

    haveBin = Sh360Fs_Init();
    SH_DebugLogInit();
    printf("log: %s\n", g_ShLogPath);
     ...
     * way to tune anything; absent, the PC defaults apply. */
    snprintf(cfgPath, sizeof(cfgPath), "%ssilenthill.cfg", Sh360Fs_DataRoot());
    ...
    /* Last chance to get the boot trace onto the stick: MainLoop never returns,
     * and if it hangs this is all the evidence there will be. */
    SH_DebugLogFlush();

...
#ifdef SH_PC_PORT
#include <stdio.h>
#include <string.h>   /* strcmp — the play-as held-item "HERO" texture test */
#include "sh_log.h"
#include "pc_config.h"
#ifdef SH_PC_PORT
...
void Ipd_CloseRangeChunksInit(void) // 0x8003C3AC
{
...
#ifdef SH_PC_PORT
    if (g_WorldEnvWork.isFogEnabled && !g_PcConfig.disableCulling)
#else
...
#endif
    {
        ...
        flagsCpy = g_WorldGfxWork.mapInfo->flags;
...
#include "sh_hwperf.h"
static inline unsigned long long ShxWdRdtsc(void) { return SH_CYCLES(); }
#endif
...
void WorldGfx_HeldItemDraw(void) // 0x8003D058
{
    ...
    if (Fs_QueueIsEntryLoaded(heldItem->queueIdx))
    {
        ...
        if (!lmHdr->isLoaded)
        {
            ...
            {
                extern void Pc_PlayAs_HeldItemImageDesc(void* imageDesc);
...
#ifdef SH_PC_PORT
    {
        extern void* Pc_PlayAs_PlayerLmRedirect(int modelFileIdx, void* lmHdr);
        extern void  Pc_PlayAs_PlayerImageDesc(void* imageDesc);

        /* A play-as retarget must never read a non-HERO ILM into the HERO-sized
         * slab: disc reads are sector-granular and HELD_ITEM_LM_BUFFER sits only
         * 14592 bytes above HARRY_LM_BUFFER, so the read would overrun into the
         * equipped weapon's PLM window. */
        harryLmHdr = (s_LmHeader*)Pc_PlayAs_PlayerLmRedirect(
            CHARA_FILE_INFOS[Chara_Harry].modelFileIdx, harryLmHdr);
        /* ...and a non-HERO player TIM must never land in the HERO-sized VRAM
         * parcel: it overruns onto the chara CLUT shelf below it and turns every
         * enemy in the map rainbow. Mutating `image` here covers both the upload
         * and the material bake — they share this desc. */
...
#ifdef SH_PC_PORT
    { extern void Pc_PlayAs_OnPlayerModelLoaded(void); Pc_PlayAs_OnPlayerModelLoaded(); }
#endif
}
...
#ifdef SH_PC_PORT
        {
            ...
            for (int _mi = 0; _mi < lm->materialCount; _mi++) {
                ...
                SH_DBG("[CHARA-TEX] mat[%d] name='%.8s' field_C=%d E=0x%02x F=0x%02x clut=0x%04x UV=0x%04x tex=%p",
                        _mi, _m->name.str, _m->field_C, _m->field_E, _m->field_F,
                        _m->field_10, _m->field_14.u16, (void*)_m->texture);
            }
            /* Log first prim's texture data from first model's first mesh */
            if (lm->modelCount > 0 && lm->modelHdrs[0].meshCount > 0) {
                s_MeshHeader* _mh = &lm->modelHdrs[0].meshHdrs[0];
                if (_mh->primitiveCount > 0) {
                    s_Primitive* _p = &_mh->primitives[0];
            ...
            SH_DBG("[CHARA-TEX] texDesc: tPage=[%d,%d] u=%d v=%d clutXY=(%d,%d)",
                    model->texture.tPage[0], model->texture.tPage[1],
                    model->texture.u, model->texture.v,
                    model->texture.clutX, model->texture.clutY);
        }
#endif
...
#endif
{
    ...
    if (timer != Q12(0.0f))
    {
        func_80055330(g_WorldEnvWork.field_0, g_WorldEnvWork.field_20, g_WorldEnvWork.field_3, tintColor.r << 5, tintColor.g << 5, tintColor.b << 5, g_WorldEnvWork.screenBrightness);
    }
}
...
void WorldGfx_HeldItemAttach(e_CharaId charaId, s32 arg1) // 0x8003DD80
{
    ...
    switch (charaId)
    {
        case Chara_Harry:
            ...
            {
                extern int  Pc_PlayAs_SuppressHarryHandVariants(void);
                extern void Pc_PlayAs_ApplySkinVisibility(void);

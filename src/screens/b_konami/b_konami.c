...
/* QOL boot-logo skip. The stock logo loops only react to `heldBtnFlags`, but
 * neither the keyboard nor the gamepad is wired into the game's controller
 * flags this early in boot on PC, so that is always 0 here and the logos can
 * never be skipped manually. Read the raw SDL keyboard + gamepad directly as a
 * fallback so any confirm/start key (or any gamepad face/start button) skips. */
...
#ifdef SH_PC_PORT
        /* QOL: any confirm/start key skips the boot logo from any visible phase.
         * The stock skip only fires during the hold-delay, so the fade-in could
         * not be skipped. Jump to the fade-out + finish; the asset loads kicked
         * off in Init are still awaited by Fs_QueueWaitForEmpty there, so this
         * only drops the on-screen logo time, never a load. */
        if (BootSkip_Pressed() &&
...
#ifdef SH_PC_PORT
        /* QOL: any confirm/start key (or gamepad button) skips the KCET logo.
         * Its entire visible time is the CheckMemCards fade-in (~5s on PC: the
         * Q12(0.2) timestep with dt forced to 1/60), which has no stock skip.
         * On skip, accelerate that fade-in so the memcard check + loads run
         * immediately (detection still happens — Continue/Load stay correct),
         * then collapse the post-load LogoDelay hold into a quick fade-out.

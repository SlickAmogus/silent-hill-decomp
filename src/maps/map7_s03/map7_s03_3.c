...
void func_800E514C(void) // 0x800E514C
{
    ...
    switch (g_SysWork.sysStateSteps[0])
    {
        ...
        case 31:
             ...
             * `!Sd_AudioStreamingCheck()` is the real scream-end sync). Our XA
             * player keeps the stream "active" for the whole ~8.5s playback, so
             * waiting here freeze-frames the scene at the impact pose
             * (g_Cutscene_Timer clamped at 320, so every DMS character/camera is
             * stuck) until the scream drains. The impact is done once the timer
             * reaches 320; advance then and let the scream overlap the
             * transformation, matching PSX. */

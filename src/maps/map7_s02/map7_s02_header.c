#include "bodyprog/bodyprog.h"
#include "bodyprog/math/math.h"
#include "maps/map7/map7_s02.h"
#include "maps/particle.h"
#include "maps/characters/player.h"
#include "maps/characters/bloodsucker.h"
#include "maps/characters/bloody_lisa.h"
#include "maps/characters/dahlia.h"
#include "maps/characters/ghost_child_alessa.h"
#include "maps/characters/kaufmann.h"
#include "maps/characters/lisa.h"
#include "maps/characters/stalker.h"
#include "maps/characters/ghost_doctor.h"

extern s_AnimInfo HARRY_M7S02_ANIM_INFOS[];

void (*g_LoadScreenFuncs[])() = {
    NULL,
    GameBoot_LoadScreen_PlayerRun,
    GameBoot_LoadScreen_BackgroundImg,
    GameBoot_LoadScreen_StageString
};

s_MapPoint2d MAP_POINTS[] = {
#include "map_points.h"
};

void (*g_MapEventFuncs[])() = {
    MapEvent_DoorJammed,
    MapEvent_DoorLocked,
    MapEvent_DoorUnlocked,
    MapEvent_CommonItemTake,
    func_800DB498,
    func_800DB52C,
    MapEvent_KeyOfBethorUse,
    MapEvent_KeyOfAratronUse,
    func_800DB738,
    func_800DBB34,
    MapEvent_AnkhTake,
    func_800DC14C,
    func_800DC778,
    func_800DC94C,
    func_800DC954,
    func_800DCD00,
    func_800DD2D4,
    func_800DD9E8,
    func_800DDEC8,
    func_800DF1E8,
    func_800DE1FC,
    MapEvent_DiskOfOuroborosTake,
    func_800DF21C,
    func_800DFA54,
    MapEvent_ScrewdriverTake,
    func_800DFAAC,
    func_800DFAE4,
    MapEvent_AmuletOfSolomonTake,
    func_800DA248,
    func_800DAA4C,
    func_800DFB48,
    func_800DFDDC,
    MapEvent_CrestOfMercuryTake,
    MapEvent_RingOfContractTake,
    MapEvent_CameraTake,
    func_800E0CB4,
    func_800E0FF0,
    func_800E12E4,
    func_800E1398,
    sharedFunc_800DB60C_7_s01,
    func_800E1DAC,
    func_800E1FF4,
    sharedFunc_800D15F0_3_s01,
    func_800E2DC0,
    func_800E2DEC,
    func_800E32E0
};

SH_MAP_OVERLAY_HEADER = {
    .mapInfo                        = &MAP_INFOS[MapType_ER2],
    .mapRoomIdxGet              = Map_RoomIdxGet,
    .unused_8                          = NO_VALUE,
    .func_C                           = sharedFunc_800D929C_0_s00,
    .bgmEvent                      = func_800D97FC,
    .bgmIdx                        = 1,
    .ambientAudioIdx               = 38,
    .field_16                         = 2,
    .field_17                         = 6,
    .loadingScreenFuncs            = g_LoadScreenFuncs,
    .mapPoints           = MAP_POINTS,
    .mapEventFuncs                 = g_MapEventFuncs,
    .mapEvents                     = MAP_EVENTS,
    .npcBoneCoordBuffer                         = &g_SysWork.npcBoneCoordBuffer[0],
    .loadableItems                 = LOADABLE_INVENTORY_ITEMS,
    .mapMessages                   = MAP_MESSAGES,
    .harryMapAnimInfos                     = HARRY_M7S02_ANIM_INFOS,
    .field_38                         = g_MapHeaderTable_38,
    .initWorldObjects              = Map_WorldObjectsInit,
    .updateWorldObjects            = Map_WorldObjectsUpdate,
    .func_44                          = NULL,
    .npcSpawnEvent                 = func_800E5628,
    .unkTable1_4C                     = sharedData_800DFB7C_0_s00,
    .unkTable1Count_50                = ARRAY_SIZE(sharedData_800DFB7C_0_s00),
    .bloodSplats                   = g_Effect_BloodSplats,
    .bloodSplatCount               = ARRAY_SIZE(g_Effect_BloodSplats),
    .field_5C                         = NULL,
    .func_60                          = NULL,
    .func_64                          = NULL,
    .func_68                          = NULL,
    .func_6C                          = NULL,
    .func_70                          = NULL,
    .func_74                          = NULL,
    .func_78                          = NULL,
    .field_7C                         = NULL,
    .func_80                          = NULL,
    .func_84                          = NULL,
    .func_88                          = NULL,
    .func_8C                          = NULL,
    .func_90                          = NULL,
    .field_94                         = NULL,
    .func_98                          = NULL,
    .func_9C                          = NULL,
    .ptr_A0                           = NULL,
    .func_A4                          = NULL,
    .func_A8                          = NULL,
    .func_AC                          = NULL,
    .func_B0                          = NULL,
    .func_B4                          = NULL,
    .func_B8                          = sharedFunc_800CDAA8_0_s02,
    .func_BC                          = sharedFunc_800D1C38_0_s00,
    .func_C0                          = sharedFunc_800D209C_0_s00,
    .playerMatchArmAnimDisable     = Player_MatchArmAnimDisable,
    .playerControlFreeze           = Player_ControlFreeze,
    .playerControlUnfreeze         = Player_ControlUnfreeze,
    .playerPathWaypointExecute                          = sharedFunc_800D23EC_0_s00,
    .playerAnimStateSet                          = sharedFunc_800D2C7C_0_s00,
    .playerAnimReset                          = sharedFunc_800D2D2C_0_s00,
    .playerAnimLock                = Player_AnimLock,
    .playerAnimIsLocked            = Player_AnimIsLocked,
    .playerAnimUnlock              = Player_AnimUnlock,
    .playerAnimPlaybackStateGet                          = Player_AnimPlaybackStateGet,
    .playerMoveSpeedIsZero          = Player_MoveSpeedIsZero,
    .playerMoveSpeedClear           = Player_MoveSpeedClear,
    .playerFallBackward            = Player_FallBackward,
    .func_68                          = sharedFunc_800D2E7C_0_s00,
    .playerDamageFeetFront         = Player_DamageFeetFront,
    .func_100                         = sharedFunc_800D2E8C_0_s00,
    .func_104                         = sharedFunc_800D2E6C_0_s00,
    .func_108                         = sharedFunc_800D2E94_0_s00,
    .func_10C                         = sharedFunc_800D2EB4_0_s00,
    .func_110                         = sharedFunc_800D2EF4_0_s00,
    .func_114                         = sharedFunc_800D2EA4_0_s00,
    .func_118                         = sharedFunc_800D2E9C_0_s00,
    .func_11C                         = sharedFunc_800D88AC_0_s00,
    .func_120                         = sharedFunc_800D88C0_0_s00,
    .charaAnimStateSet                         = sharedFunc_800D8904_0_s00,
    .charaAnimReset                         = Player_RunTimerReset,
    .charaAnimLock                         = Chara_Lock,
    .charaAnimIsLocked                         = Chara_IsLockedCheck,
    .charaAnimUnlock                         = Chara_Unlock,
    .charaAnimPlaybackStateGet                         = Chara_AnimPlaybackStateGet,
    .charaPathWaypointExecute                         = sharedFunc_800D8A00_0_s00,
    .charaVisibleSet                         = Chara_VisibleSet,
    .charaInvisibleSet                         = Chara_InvisibleSet,
    .func_148                         = sharedFunc_800D908C_0_s00,
    .func_14C                         = sharedFunc_800D9188_0_s00,
    .charaAnimStartKeyframeIdxGet      = Chara_AnimStartKeyframeIdxGet,
    .func_104                         = Chara_CollisionReset,
    .func_158                         = NULL,
    .func_15C                         = NULL,
    .func_160                         = NULL,
    .func_164                         = NULL,
    .particlesUpdate              = Particle_SystemUpdate,
    .enviromentSet                = sharedFunc_800D08B8_0_s00,
    .func_170                         = sharedFunc_800D0A60_0_s00,
    .func_174                         = sharedFunc_800D0B18_0_s00,
    .particleHyperBlasterBeamDraw = Particle_HyperBlasterBeamDraw,
    .particleBeamDraw             = Particle_BeamDraw,
    .particleSoundStop            = Particle_SoundStop,
    .windSpeedX                   = &g_Particle_SpeedX,
    .windSpeedZ                   = &g_Particle_SpeedZ,
    .data_18C                         = &sharedData_800DFB6C_0_s00,
    .data_190                         = &sharedData_800DFB70_0_s00,
    .charaUpdateFuncs             = {
        NULL, NULL, NULL, NULL, NULL, NULL, NULL, Stalker_Update,
        Stalker_Update, Stalker_Update, NULL, NULL, NULL, NULL, NULL, NULL,
        NULL, NULL, NULL, NULL, NULL, Bloodsucker_Update, NULL, NULL,
        NULL, NULL, NULL, NULL, NULL, NULL, Dahlia_Update, Dahlia_Update,
        Lisa_Update, BloodyLisa_Update, NULL, GhostChildAlessa_Update, NULL, NULL, Kaufmann_Update, Kaufmann_Update,
        NULL, NULL, GhostDoctor_Update },
    .charaGroupIds = { Chara_Stalker },
    .charaSpawnInfos   = {
#include "chara_spawns.h"
    },
    .cameraPaths = {
#include "vc_road_data.h"
    },
    .collisionTriggers = {
#include "header_field_D2C.h"
    },
};
